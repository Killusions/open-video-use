...
3. An OpenAI API key in `.env` at the repo root (for Whisper transcription). Optionally `OPENAI_BASE_URL` and `WHISPER_MODEL` for OpenAI-compatible endpoints.
...
Whisper (via OpenAI-compatible API) does all transcription. Without a key, nothing transcribes.
...
1. Check existing state in this order and stop at the first hit:
    ...
    grep -q '^OPENAI_API_KEY=..' ~/Developer/video-use/.env 2>/dev/null && echo "dotenv"
    ```

2. If neither is set, ask the user exactly once:

    > I need an OpenAI API key for Whisper transcription (word-level timestamps). Grab one at https://platform.openai.com/api-keys and paste it here -- I'll write it to `~/Developer/video-use/.env`. Or if you already have it exported as `OPENAI_API_KEY`, say "use env" and I'll skip. If you're using a different OpenAI-compatible endpoint, also provide the base URL.
...
3. Sanity check with a cheap, quota-free call:
    ...
    curl -s -o /dev/null -w '%{http_code}\n' \
      -H "Authorization: Bearer $(sed -n 's/^OPENAI_API_KEY=//p' ~/Developer/video-use/.env)" \
...
Full transcription test is optional at install time -- it uses Whisper API credits. Better to wait until the user hands you their first clip.
...
- Never run transcription as part of install verification unless the user explicitly asks — Whisper API calls cost real money.
